...
def test_fetch_news_range(mock_mongo_client):
    """Test MongoDB news fetch between time range with cursor + sort + batch_size."""

    now = datetime(2025, 10, 13, 0, 0, tzinfo=timezone.utc)
    # Include the fields that the real projection uses: releasedAt, assets, content, news_count
    docs = [
        {
            "releasedAt": now,
            "content": "Some news text",
            "news_count": 1,
            "assets": [{"symbol": "BINANCE:BTCUSDT"}],
        }
    ]
    ...
    class MockCursor:
        ...
        def sort(self, *args, **kwargs):
            # Accept ("releasedAt", 1) and return self for chaining
            return self

        def batch_size(self, *args, **kwargs):
            # Accept (10000,) and return self for chaining
            return self
    ...
    mock_collection.find.return_value = MockCursor(docs)

    # mongo_client[MO.MONGO_DB][MO.MONGO_COLLECTION] → mock_db[collection_name]
    mock_db = MagicMock()
...
def test_save_predictions(mock_mongo_col):
    """Test saving predictions to Mongo (Redis is handled internally)."""
    from apps.NeuralFusionCore.scripts.prediction_service import save_predictions
    ...
    predictions = [
        {"ts": dt.utcnow(), "weights": weights, "stocks": stocks}
    ]

    save_predictions(predictions)

    # Verify some Mongo insert was attempted
...
def test_prediction_history(mock_collection):
    ...
    mock_cursor = [
        {
            "ts": datetime(2025, 10, 13, 0, 0),
            "weights": [0.1, 0.2],
            "stocks": ["BTCUSDT"],
        },
        {
            "ts": datetime(2025, 10, 13, 1, 0),
            "weights": [0.2, 0.8],
            "stocks": ["ETHUSDT"],
        },
    ]
    # Simulate collection.find(...).sort(...).limit(...)
